...
def pred_image(net, img_, output, output_gray):
    img_src = Image.open(img_)
    img = np.asarray(img_src.resize((img_size, img_size)).convert('RGB')).astype(np.float32)
    size = img_src.size
...
def test_my(input_bands, model_name, model_dir, img_size, num_class):
    ...
    for i in tqdm.tqdm(range(len(imgs))):
        pred_image(net, os.path.join(path, imgs[i]), output, output_gray)
    gt = './data/label_val/'
    iou, miou, acc, prec, rec, f1_s = evaluate(output_gray, gt, num_class)
    return iou, miou, acc, prec, rec, f1_s
