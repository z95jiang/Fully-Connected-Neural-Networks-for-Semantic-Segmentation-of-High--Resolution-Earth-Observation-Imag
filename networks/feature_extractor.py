...
class ResNet(nn.Module):
    def __init__(self, input_bands, block, layers=(3, 4, 23, 3)):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(input_bands, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def resnet18(input_bands,pretrained=True):
    model = ResNet(input_bands,BasicBlock, [2, 2, 2, 2])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(input_bands,pretrained=True):
    model = ResNet(input_bands,BasicBlock, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(input_bands,pretrained=True):
    model = ResNet(input_bands,Bottleneck, [3, 4, 6, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(input_bands,pretrained=True):
    model = ResNet(input_bands,Bottleneck, [3, 4, 23, 3])
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152(input_bands,pretrained=True):
    model = ResNet(input_bands,Bottleneck, [3, 8, 36, 3])
    if pretrained:
