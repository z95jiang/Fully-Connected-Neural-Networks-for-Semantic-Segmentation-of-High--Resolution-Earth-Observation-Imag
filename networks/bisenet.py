...
class FeatureFusionModule(torch.nn.Module):
    def __init__(self, n_classes, in_channels):
        ...
        self.convblock = ConvBlock(in_channels=self.in_channels, out_channels=n_classes, stride=1)
        self.conv1 = nn.Conv2d(n_classes, n_classes, kernel_size=1)
        self.relu = nn.ReLU()
...
class BiSeNet(torch.nn.Module):
    def __init__(self, n_classes, context_path):
        super().__init__()
        # build spatial path
        ...
        if context_path == 'resnet101':
            ...
            # supervision block
            self.supervision1 = nn.Conv2d(in_channels=1024, out_channels=n_classes, kernel_size=1)
            self.supervision2 = nn.Conv2d(in_channels=2048, out_channels=n_classes, kernel_size=1)
            # build feature fusion module
            self.feature_fusion_module = FeatureFusionModule(n_classes, 3328)
