...
class Atrous_ResNet_features(nn.Module):
    ...
    def __init__(self, block, layers, pretrained=False):
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=1, rate=1)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1, rate=1)
...
class DeepLabv3(nn.Module):
    ...
    def forward(self, x0):
        ...
        x = self.fc1(x)
        x = self.fc2(x)
        x = F.upsample(x, size=(x0.size()[2], x0.size()[3]), mode='bilinear')
